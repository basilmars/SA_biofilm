...
# 读取表格
biofilm_genes <- read_excel("D:/NetDrive/OneDrive - wlulab/Work/复旦/课题/MRSA biofilm vs HNP1/20241114 - RNAseq - 29213 VS HNP1/gene_list_biofilm.xlsx")
...
library(writexl)
write_xlsx(merged_data, "H_vs_S_with_gene.xlsx")

# 根据 biofilm Gene 提取 merged_data 中的匹配信息
biofilm_gene_data <- merged_data %>%
  filter(gene %in% biofilm_genes$Gene)

write_xlsx(biofilm_gene_data, "biofilm_gene_data.xlsx")
...
# 读取提取的数据
biofilm_gene_data <- read_excel("./biofilm_gene_data.xlsx")

extracted_data <- biofilm_gene_data
...
annotation_colors <- list(
  Group = c(Control = "lightgoldenrodyellow", HNP1 = "palegreen"),
