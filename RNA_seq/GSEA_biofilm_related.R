...
# ??????????????????
rpkm_data <- read_excel("D:/NetDrive/OneDrive - wlulab/Work/??????/??????/MRSA biofilm vs HNP1/20241114 - RNAseq - 29213 VS HNP1/H_vs_S_with_gene.xlsx")
...
# ???????????????????????????????????????????????? gene_list_biofilm.xlsx???
biofilm_genes <- read_excel("./biofilm_gene_data.xlsx")
biofilm_gene_list <- as.character(biofilm_genes$gene)  # ??????????????? "Gene"
...
gsea_res <- GSEA(
  ...
  TERM2GENE = data.frame(
    term = "Biofilm-Related Genes",
    gene = biofilm_gene_list
  ),
  pvalueCutoff = 1,
  minGSSize = 10,
  maxGSSize = 500,
...
#----?????????????????????-----------
#?????? https://www.jianshu.com/p/e1e099049f93
...
# ?????? logFC ??????????????? 1 ??? FDR ?????? 0.05 ?????????
mygene <- c("eap") #biofilm_genes %>%  filter(abs(logFC) > 1 & FDR < 0.05) %>%  pull(gene)  # ?????????????????????
